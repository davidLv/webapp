...
class AdminController extends CommonController {
    ...
    public function public_editInfo($info = array()){
        ...
        if (IS_POST){
            ...
            $state = $admin_db->where(array('id'=>$userid))->save($info);
            $state ? $this->success('修改成功') : $this->error('修改失败');
        }else {
            $menu_db = D('Menu');
            $currentpos = $menu_db->currentPos(I('get.menuid'));  //栏目位置
            $info = $admin_db->where(array('id'=>$userid))->find();
    ...
    public function public_editPwd(){
        ...
        $admin_db = D('Admin');
        if(IS_POST){
            $info = $admin_db->where(array('id'=>$userid))->field('password,encrypt')->find();
    ...
    public function loginLog($page=1, $rows=10, $search = array(), $sort = 'time', $order = 'desc'){
        if(IS_POST){
            ...
            //搜索
            $where = array("`type` = 'login'", "`user_id` = {$userid}");
    ...
    public function loginLogDelete($week = 4){
        ...
        $d = date('Y-m-d', $start);
        $where = "`type` = 'login' AND `id` = {$userid} AND left(`time`, 10) <= '$d'";
     ...
     * 用户管理
     */
    public function memberList($page = 1, $rows = 10, $sort = 'id', $order = 'asc'){
        if(IS_POST){
            $admin_db = D('Admin');
            ...
            $order = $sort.' '.$order;
            $limit = ($page - 1) * $rows . "," . $rows;
            $list = $total ? $admin_db->field('id,user_name,last_login_ip,email,real_name,last_login_time,role_id')->order($order)->limit($limit)->select() : array();
    ...
    public function memberAdd(){
        if(IS_POST){
            $admin_db = D('Admin');
            $data = I('post.info');
            if($admin_db->where(array('user_name'=>$data['username']))->field('user_name')->find()){
                $this->error('用户名称已经存在');
            }
            ...
            $data['id'] = str_replace(array('{','}'), '', String::uuid());
    ...
    public function memberEdit($id){
        ...
        if(IS_POST){
            ...
            $result = $admin_db->where(array('id'=>$id))->save($data);
            if($result){
                $this->success('修改成功');
            }else {
                $this->error('修改失败');
    ...
    public function memberResetPassword($id){
        ...
        if($email){
            $userInfo = $admin_db->field('username,email')->where(array('id'=>$id))->find();
            $email = array_merge($email, array(
                'email' => $userInfo['email'],
                'content' => str_replace(array('{username}', '{password}', '{site}'), array($userInfo['user_name'], $password, SITE_URL), htmlspecialchars_decode($email['content']))
            ));
        }
    ...
    public function memberDelete($id){
        ...
        $admin_db = D('Admin');
        $result = $admin_db->where(array('id'=>$id))->delete();
        if ($result){
            $this->success('删除成功');
        }else {
            $this->error('删除失败');
    ...
    public function public_checkName($name){
        ...
        $admin_db = D('Admin');
        $exists = $admin_db->where(array('user_name'=>$name))->field('user_name')->find();
        if ($exists) {
            exit('false');
        }else{
            exit('true');
